...
- Adds a warning when `style` and `script` tags are allowed, as they are inherently vulnerable. That warning can be disabled by including the option `allowVulnerableTags: true` so this choice is knowing and explicit.
